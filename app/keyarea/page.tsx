...
function ChapterModuleForm() {
  ...
  const handleCreateKeyAreaSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        setError("");
        const newKeyArea = await response.json();
        setKeyAreas((prevKeyAreas) => [...prevKeyAreas, newKeyArea]);
  ...
  const handleCreateChapterSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        setError("");
        const updatedKeyArea = await response.json();
        setKeyAreas((prevKeyAreas) =>
          prevKeyAreas.map((keyArea) => (keyArea.areaID === updatedKeyArea.areaID ? updatedKeyArea : keyArea))
        );
        formRef.current?.reset();
    ...
    <div className="flex flex-col items-center justify-start min-h-screen rounded-lg shadow-2xl">
      <div className="bg-white w-full h-fit mx-auto p-8 space-y-8 rounded-xl shadow items-center justify-center text-center">
        <div className="bg-white container mx-auto p-4 h-fit flex flex-col items-center justify-center text-center space-y-4">
          <h1 className="text-4xl text-center font-semibold mb-3">Key Areas, Chapters, and Modules</h1>
          <ul className="w-full space-y-4">
            {keyAreas.map((keyArea) => (
              <li key={keyArea.areaID} className="bg-gray-100 p-4 rounded-lg shadow-md">
                      ...
                      <ul className="mt-4">
                        {chapter.modules.map((module) => (
                          <li key={module.moduleID} className="mt-2">
                            <h4 className="text-md font-semibold">{module.moduleName}</h4>
